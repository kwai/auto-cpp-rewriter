...
void CtorVisitor::visit(clang::CXXConstructorDecl* cxx_constructor_decl, FeatureInfo* feature_info_ptr) {
  ...
  LOG(INFO) << "get feature type: " << feature_type << ", class_name: " << feature_name;
  ...
  if (cxx_constructor_decl->hasBody()) {
    ...
    if (clang::Stmt* body = cxx_constructor_decl->getBody()) {
      recursive_visit(body, feature_info_ptr, &env);
    }
...
void CtorVisitor::recursive_visit(clang::Stmt* stmt, FeatureInfo* info_ptr, Env* env_ptr) {
  if (stmt == nullptr || info_ptr == nullptr || env_ptr == nullptr) {
    return;
  }
  ...
  if (clang::CompoundStmt* compound_stmt = dyn_cast<clang::CompoundStmt>(stmt)) {
    for (clang::CompoundStmt::body_iterator start = compound_stmt->body_begin();
      ...
      recursive_visit(*start, info_ptr, env_ptr);
    }
    ...
    recursive_visit(expr_with_cleanups->getSubExpr(), info_ptr, env_ptr);

  } else if (clang::ImplicitCastExpr* implicit_cast_expr = dyn_cast<clang::ImplicitCastExpr>(stmt)) {
    recursive_visit(implicit_cast_expr->getSubExpr(), info_ptr, env_ptr);
    ...
    for (unsigned i = 0; i < cxx_member_call_expr->getNumArgs(); i++) {
      recursive_visit(cxx_member_call_expr->getArg(i), info_ptr, env_ptr);
    }
    process(cxx_member_call_expr, info_ptr, env_ptr);
    ...
    for (size_t i = 0; i < cxx_operator_call_expr->getNumArgs(); i++) {
      recursive_visit(cxx_operator_call_expr->getArg(i), info_ptr, env_ptr);
    }

  } else if (clang::UnaryOperator* unary_operator = dyn_cast<clang::UnaryOperator>(stmt)) {
    recursive_visit(unary_operator->getSubExpr(), info_ptr, env_ptr);
    ...
    recursive_visit(binary_operator->getLHS(), info_ptr, env_ptr);
    recursive_visit(binary_operator->getRHS(), info_ptr, env_ptr);
    ...
    recursive_visit(materialize_temporary_expr->getSubExpr(), info_ptr, env_ptr);
    ...
    for (size_t i = 0; i < cxx_construct_expr->getNumArgs(); i++) {
      recursive_visit(cxx_construct_expr->getArg(i), info_ptr, env_ptr);
    }
    ...
    recursive_visit(cond_expr->IgnoreImpCasts(), info_ptr, &new_env);
    ...
    for (auto it = for_stmt->child_begin(); it != for_stmt->child_end(); it++) {
      recursive_visit(*it, info_ptr, env_ptr);
    }
...
void CtorVisitor::process(clang::CXXMemberCallExpr* cxx_member_call_expr,
                          FeatureInfo* feature_info_ptr,
                          Env* env_ptr) {
  if (cxx_member_call_expr == nullptr || feature_info_ptr == nullptr || env_ptr == nullptr) {
    return;
  }
